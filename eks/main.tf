...
module "eks_cluster" {
  ...
  eks_managed_node_groups = {
    eks_managed_node_group = {
      instance_types = ["${local.workspace.eks_cluster.instance_type}"]
      desired_capacity = try(local.workspace.eks_cluster.min_size, 2)
      max_capacity = try(local.workspace.eks_cluster.max_size, 5)
      min_capacity = try(local.workspace.eks_cluster.min_size, 2)
      key_name = local.workspace.eks_cluster.key_name
      subnets = data.aws_subnets.private.ids
      additional_security_group_ids = local.workspace.eks_cluster.node_security_group
    }
  }
